...
##
## Script name: Script_File_S6_Data_analysis_ELOA_Validation.R
##
## Purpose of script: Clean up MaxQuant data and analyse
## Please be aware that TCEB3 (as output by MaxQuant) is synonymous to ELOA and 
## was changed here to harmonize gene names throughout the manuscript
##
...
meanssd.file   <- "MeansSDPlot_ELOA.png"
qq.file.1      <- "QQ_plot_ELOA_1.png"
vsn.tmt.file.1 <- "VSN_intensities_TMT_ELOA_1.png" 
box.file.1     <- "Boxplots_ELOA_1.png"
stats.file     <- "Statistics_ELOA.csv"
data.file.1    <- "Data_ELOA_1.csv"

## Read in data ----

message("Reading data for ELOA")
...
peptides.df <- peptides.df[which(peptides.df$Gene.names == "TCEB3"), ]
...
## Extract ELOA (TCEB3) data

data <- data[grep("TCEB\\.1", data$Experiment), ]
data <- data[which(data$Gene.names == "TCEB3"), ]
...
message("VSN transformed TMT intensities plot ELOA")
...
colnames(data.1.quant) <- c(
  ...
  "ELOA+CDKL5_WT+H2O2.1",
  "ELOA+CDKL5_WT+H2O2.2",
  "ELOA+CDKL5_WT+H2O2.3",
  "ELOA+CDKL5_K42R+H2O2.1",
  "ELOA+CDKL5_K42R+H2O2.2",
  "ELOA+CDKL5_K42R+H2O2.3"
)

p.pep.soi.1.rows <- which(data.1.quant$Modifications == "Phospho (STY)")
data.1.quant <- data.1.quant[p.pep.soi.1.rows , ]

## First peptide is different AA sequence to second, remove first as missing most datapoints

data.1.quant <- data.1.quant[which(data.1.quant$Modified.Sequence != "_EENRRPPpSGDNAR_"), ]

## All probabilities for phospho-site assignment == 1, nothing to exclude.
...
wt.1   <- grep("ELOA\\+CDKL5_WT\\+H2O2", names(data.1.av))
k42r.1 <- grep("ELOA\\+CDKL5_K42R\\+H2O2", names(data.1.av))
...
## Two t-tests: α = 0.05/2 = 0.025, all pass

## Visualization ----

message("Boxplots ELOA")
...
data.1.gg$Group <- factor(
  data.1.gg$Group,
  levels = c("FLAG+CDKL5_WT+H2O2","ELOA+CDKL5_WT+H2O2", "ELOA+CDKL5_K42R+H2O2")
)
...
ggplot(
  ...
  geom_point(size = 3) +
  labs(title = paste("ELOA - phospho-Serine 311 -", peptide.1)) +
  ylab("VSN Intensity") +
  theme(
    axis.text.x = element_text(family = "Arial", size = 14),
    axis.title = element_text(size = 12),
