...
##
## Script name: Script_File_S7_Data_analysis_TTDN1_Validation.R
##
## Purpose of script: Clean up MaxQuant data and analyse
## Please be aware that MPLKIP (as output by MaxQuant) is synonymous to TTDN1 and 
...
meanssd.file   <- "MeansSDPlot_TTDN1.png"
qq.file.1      <- "QQ_plot_TTDN1_1.png"
vsn.tmt.file.1 <- "VSN_intensities_TMT_TTDN1_1.png" 
box.file.1     <- "Boxplots_TTDN1_1.png"
stats.file     <- "Statistics_TTDN1.csv"
data.file.1    <- "Data_TTDN1_1.csv"
...
message("Reading data for TTDN1")
...
peptides.df <- peptides.df[which(peptides.df$Gene.names == "MPLKIP"), ]
...
data <- data[grep("MPLKIP\\.1", data$Experiment), ]
data <- data[which(data$Gene.names == "MPLKIP"), ]
...
message("VSN transformed TMT intensities plot TTDN1")
...
colnames(data.1.quant) <- c(
  ...
  "TTDN1+CDKL5_WT+H2O2.1",
  "TTDN1+CDKL5_WT+H2O2.2",
  "TTDN1+CDKL5_WT+H2O2.3",
  "TTDN1+CDKL5_K42R+H2O2.1",
  "TTDN1+CDKL5_K42R+H2O2.2",
  "TTDN1+CDKL5_K42R+H2O2.3"
)

p.pep.soi.1.rows <- which(data.1.quant$Modifications == "Phospho (STY)")
data.1.quant <- data.1.quant[p.pep.soi.1.rows , ]

na.rows <- which(rowSums(data.1.quant[, 4:12], na.rm = TRUE) == 0)
data.1.quant <- data.1.quant[-na.rows, ]
...
wt.1   <- grep("TTDN1\\+CDKL5_WT\\+H2O2", names(data.1.av))
k42r.1 <- grep("TTDN1\\+CDKL5_K42R\\+H2O2", names(data.1.av))
...
data.1.gg$Group <- factor(
  data.1.gg$Group,
  levels = c("FLAG+CDKL5_WT+H2O2","TTDN1+CDKL5_WT+H2O2", "TTDN1+CDKL5_K42R+H2O2")
)
...
ggplot(
  ...
  geom_point(size = 3) +
  labs(title = paste("TTDN1 - phospho-Serine 40 -", peptide.1)) +
  ylab("VSN Intensity") +
  theme(
    axis.text.x = element_text(family = "Arial", size = 14),
    axis.title = element_text(size = 12),
